...
def get_results(profile, hours, uptime, types, region, product):
    #print "Calling for region " + str(region)
    result={}
    for it, bid in types.items():
        result.update(get_last_spot_price_exceeding_the_bid(profile, hours, uptime,it, region, product, bid))
    return result

def func_star(a_b):
    """Convert `f([1,2])` to `f(1,2)` call."""
    return get_results(*a_b)

# For each availability zone, return timestamp of the last time Spot price exceeded specified bid price
def get_last_spot_price_exceeding_the_bid(profile, hours, uptime,inst_type, region, product, bid):
    now = datetime.datetime.utcfromtimestamp(time.time())

    required_uptime = now - datetime.timedelta(hours=uptime)
    required_uptime_unix = calendar.timegm(required_uptime.utctimetuple())
    ...
    for p in res['SpotPriceHistory']:
        ...
        old_ts = max_price.get((inst_type, cur_az), None)
        if old_ts is None and cur_ts > required_uptime_unix:
            max_price[(inst_type, cur_az)] = max_bid = float(p['SpotPrice']) 

        if float(p['SpotPrice']) > max_bid and cur_ts > old_ts and cur_ts > required_uptime_unix:
            max_price[(inst_type, cur_az)] = float(p['SpotPrice'])
    return max_price
...
parser.add_argument("--uptime", dest="uptime", type=int, default=7*24, help="The number of possible uptime you want without likely interruption for your instances. Default is 1 week. Change the default value of the period to check according the value of this option i.e. if uptime > 1 week required, change the period to check as well")
...
p= Pool(processes=len(regions))            # start 4 worker processes
r = p.map(func_star,itertools.izip([args.profile]*len(regions), itertools.repeat(args.hours),itertools.repeat(args.uptime),itertools.repeat(types),regions,itertools.repeat(args.product)))
p.terminate()
...
print "Max bid Pricing in the last " + str(args.uptime) + " hours for the selected instance types"
sorted_list.sort(key=lambda x: (result.get(x, 0), x))
print "Max Bid\tInstance Type\tAvailability Zone"
for it in sorted_list:
    price = result.get(it, 0)
    print "%.4f\t%s\t%s" % (price, it[0], it[1])
